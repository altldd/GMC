...
if __name__ == '__main__':
    ...
    father_count = 0
    father_false = 0
    Output_logits = np.zeros((9400, 2, len(label_dict)))
    index = 0
    st = time.time()
    with torch.no_grad(), tqdm(test) as pbar:
        for batch in pbar:
            ...
            Output_logits[index:index+logits.shape[0], :, :] = logits.cpu().numpy()
            index = index + logits.shape[0]
            # if (Output_logits is None):太慢了！
            #     Output_logits = logits.cpu().numpy()
            # else:
            #     Output_logits = np.concatenate([Output_logits, logits.cpu().numpy()], axis = 0)
    ...
    f = h5py.File('result.h', 'w')
    f['h'] = Output_logits[:index, :, :]
    f.close()
